...
@Singleton
class SimpleMixer @Inject constructor(
    ...
    private fun loadLexicon(path: String): List<LexItem> = try {
    ...
    private fun loadSuffixes(path: String): List<String> = try {
        context.assets.open(path).bufferedReader().useLines { lines ->
    ...
    private fun blend(a: String, b: String): String {
        ...
        var best = candidates.map { it to scoreCandidate(it, aa, bb) }.maxByOrNull { it.second }?.first
            ?: headTail(aa, bb)
    ...
    private fun overlapMerge(a: String, b: String): String? {
    ...
    private fun headTail(a: String, b: String): String {
        val headMin = 4
        val tailMin = 4
    ...
    private fun linkWithVowel(left: String, right: String): String {
    ...
    private fun scoreCandidate(c: String, a: String, b: String): Int {
        var score = 0
        val s = c.lowercase()
    ...
    private fun isPronounceable(s: String): Boolean {
        ...
        for (ch in s) {
            ...
            if (ch.lowercaseChar() in vowels) { consRun = 0; hasVowel = true } else consRun++
    ...
    private fun lcsLen(x: String, y: String): Int {
        val m = x.length; val n = y.length
        var best = 0
