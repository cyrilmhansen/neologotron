...
class SettingsRepository
    ...
    constructor(
        ...
        val definitionMode: Flow<GeneratorRules.DefinitionMode> =
            context.settingsStore.data.map { p ->
                when (p[defModeKey]) {
        ...
        suspend fun setTheme(style: ThemeStyle) {
            context.settingsStore.edit { it[themeKey] = style.name }
        }

        suspend fun setDarkTheme(enabled: Boolean) {
            context.settingsStore.edit { it[darkKey] = enabled }
        }

        suspend fun setDefinitionMode(mode: GeneratorRules.DefinitionMode) {
            context.settingsStore.edit { it[defModeKey] = mode.name }
        }

        suspend fun setCoherenceFilters(enabled: Boolean) {
            context.settingsStore.edit { it[filtersKey] = enabled }
        }

        suspend fun setShakeToGenerate(enabled: Boolean) {
            context.settingsStore.edit { it[shakeKey] = enabled }
        }

        suspend fun setHapticOnShake(enabled: Boolean) {
            context.settingsStore.edit { it[hapticOnShakeKey] = enabled }
        }

        suspend fun setShakeHintShown(shown: Boolean = true) {
            context.settingsStore.edit { it[shakeHintShownKey] = shown }
        }
        ...
        suspend fun setWeightingIntensity(value: Float) {
            context.settingsStore.edit { it[weightingIntensityKey] = value }
        }

        suspend fun setAnimatedBackgroundsEnabled(enabled: Boolean) {
            context.settingsStore.edit { it[animatedBackgroundsKey] = enabled }
        }
        ...
        suspend fun setSimpleMixerEnabled(enabled: Boolean) {
            context.settingsStore.edit { it[simpleMixerKey] = enabled }
        }
    }
