    ...
    androidx.compose.material3.Scaffold(
        ...
        Column(
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                }, enabled = uiState !is UiState.Loading) { Text(text = stringResource(id = R.string.action_select_prefix)) }
                Button(onClick = {
                    showRootPicker = true
                }, enabled = uiState !is UiState.Loading) { Text(text = stringResource(id = R.string.action_select_root)) }
                Button(onClick = {
                    showSuffixPicker = true
                }, enabled = uiState !is UiState.Loading) { Text(text = stringResource(id = R.string.action_select_suffix)) }
            }
            ...
            if (selectedPrefix != null || selectedRoot != null || selectedSuffix != null) {
                Text(
                    text = "Sélection: " + listOfNotNull(selectedPrefix?.form, selectedRoot?.form, selectedSuffix?.form).joinToString(" + "),
                )
                Spacer(modifier = Modifier.height(8.dp))
            }
            Text(
            ...
            Button(onClick = {
                if (canCompose) vm.commitAndOpen(onOpenDetail)
            }, enabled = canCompose) { Text(text = stringResource(id = R.string.action_compose_word)) }
            Spacer(modifier = Modifier.height(8.dp))
            Button(onClick = {
                if (canCompose) {
            ...
            if (uiState is UiState.Loading) {
                repeat(3) { Text(text = "• ", modifier = Modifier.height(12.dp)) }
            } else {
                prefixes.take(5).forEach {
                        item ->
                    Text(text = "• ${item.form} (${item.gloss})")
                }
            }
            Spacer(modifier = Modifier.height(12.dp))
